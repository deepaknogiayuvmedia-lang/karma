...
class SMS_module
{
    public static function send($receiver, $otp)
    {
        ...
        if (isset($config) && $config['status'] == 1) {
            $response = self::releans(substr($receiver,3), $otp);
            return $response;
        }
...
// soumya technology api start //
    public static function releans($receiver, $otp)
    {
        ...
        if (isset($config) && $config['status'] == 1) {
            ...
            try {
                curl_setopt_array($curl, array(
                    CURLOPT_URL => "http://login.quicksms.info/api/sendsms?apikey=26c9f1097e1e4b2f99c56ea6552b12c0&number=$to&sendername=GROSTP&service=TRANS&msg=Hello%20Sir,%20Your%20generated%20otp%20is%20:%20$otp,%20Team%C2%A0Well%C2%A0I%C2%A0Shopping",
                    CURLOPT_RETURNTRANSFER => true,
                    CURLOPT_ENCODING => "",
                    CURLOPT_TIMEOUT => 0,
