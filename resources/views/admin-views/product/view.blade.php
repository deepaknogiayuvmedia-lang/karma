        ...
        <div class="page-header pb-0 mb-0 border-0">
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-10 mb-3">
                <div>
                    <h2 class="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
                        <img width="20" onerror="this.src='{{asset('assets/back-end/img/160x160/img2.jpg')}}'"
                             src="{{asset('/assets/back-end/img/inhouse-product-list.png')}}" alt="">
                        {{$product['name']}}
                    </h2>
                </div>
                <div class="d-flex justify-content-end flex-wrap gap-10">
                    ...
                    <a href="{{route('product',$product['slug'])}}" class="btn btn--primary" target="_blank">
                        <i class="tio-globe"></i> {{ \App\CPU\translate('View from Website') }}
                    </a>
                </div>
            </div>
                ...
                <div class="card mb-3 mb-lg-5">
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <span class="text-muted">{{\App\CPU\translate('Added By')}}:</span>
                                    <p class="mb-0"><span class="badge badge-soft-info">{{ ucfirst($product->added_by) }}</span></p>
                                </div>
                                @if($product->added_by == 'seller')
                                    @php $seller = \App\Model\Seller::find($product->user_id); @endphp
                                    @if($seller)
                                        <div class="mb-3">
                                            <span class="text-muted">{{\App\CPU\translate('Seller Name')}}:</span>
                                            <p class="mb-0">{{ $seller->f_name }} {{ $seller->l_name }}</p>
                                        </div>
                                        <div class="mb-3">
                                            <span class="text-muted">{{\App\CPU\translate('Shop Name')}}:</span>
                                            <p class="mb-0">{{ $seller->shop_name ?? '-' }}</p>
                                        </div>
                                    @endif
                                @endif
                                <div class="mb-3">
                                    <span class="text-muted">{{\App\CPU\translate('Request Status')}}:</span>
                                    <p class="mb-0">
                                        @if($product->request_status == 0)
                                            <span class="badge badge-soft-warning">{{\App\CPU\translate('New Request')}}</span>
                                        @elseif($product->request_status == 1)
                                            <span class="badge badge-soft-success">{{\App\CPU\translate('Approved')}}</span>
                                        @elseif($product->request_status == 2)
                                            <span class="badge badge-soft-danger">{{\App\CPU\translate('Denied')}}</span>
                                        @endif
                                    </p>
                                </div>
                                <div class="mb-3">
                                    <span class="text-muted">{{\App\CPU\translate('Status')}}:</span>
                                    <p class="mb-0">
                                        @if($product->status == 1)
                                            <span class="badge badge-soft-success">{{\App\CPU\translate('Active')}}</span>
                                        @else
                                            <span class="badge badge-soft-danger">{{\App\CPU\translate('Inactive')}}</span>
                                        @endif
                                    </p>
                                </div>
                            </div>
