                ...
                <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <small class="text-muted">
                            <span class="badge badge-soft-danger mr-1">Changed</span>
                            <span class="badge badge-soft-secondary">Unchanged</span>
                        </small>
                    </div>
                    <div class="card-body">
                                        ...
                                        @foreach ($allKeys as $key)
                                            @if (!in_array($key, $skipKeys))
                                                ...
                                                <tr class="{{ $isChanged ? 'table-warning' : '' }}">
                                                    <td><strong>
                                                        @php
                                                            $labelMap = ['purchase_price' => 'Market price'];
                                                    ...
                                                    <td class="text-danger">
                                                        @php
                                                            ...
                                                            // Check any HTML tag
                                                            $isHtml =
                                                                $oldDisplayStr !== strip_tags($oldDisplayStr) ||
                                                                str_contains($oldDisplayStr, '<img');
                                                        @endphp

                                                        @if ($isHtml) <div class="html-preview">
                                                                {!! $oldDisplayStr !!}
                                                            </div>
                                                        @else
                                                            {{ \Illuminate\Support\Str::limit($oldDisplayStr, 100) }} @endif
                                                    </td>
                                                    <td class="text-success">
                                                        @php
                                                            ...
                                                            // Check any HTML tag
                                                            $isHtml =
                                                                $newDisplayStr !== strip_tags($newDisplayStr) ||
                                                                str_contains($newDisplayStr, '<img');
                                                        @endphp
                                                        @if ($isHtml) <div class="html-preview">
                                                            {!! $newDisplayStr !!}
                                                        </div> 
                                                        @else
                                                            {{ \Illuminate\Support\Str::limit($newDisplayStr, 100) }} @endif
                                                    </td>
                                                    <td>
                                                        @if ($isChanged)
                                                            <span
                                                                class="badge badge-soft-danger">{{ \App\CPU\translate('Changed') }}</span>
                                                        @else
                                                            <span
                                                                class="badge badge-soft-secondary">{{ \App\CPU\translate('Unchanged') }}</span>
                                                        @endif
                                                    </td>
                                                </tr>
                                            @endif
