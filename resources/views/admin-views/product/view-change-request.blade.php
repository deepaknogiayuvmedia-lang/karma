                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                <p><strong>{{ \App\CPU\translate('Date') }}:</strong> {{ $changeRequest->created_at->format('d M Y, h:i A') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        @foreach($changeRequest->new_data as $key => $newVal)
                                            @if($key == 'temp_images') @continue @endif
                                            @php $oldVal = $changeRequest->old_data[$key] ?? ''; @endphp
                                            @if($oldVal != $newVal)
                                                <tr>
                                                    <td><strong>{{ ucfirst(str_replace('_', ' ', $key)) }}</strong></td>
                                                    <td class="text-danger">
                                                        @if(in_array($key, ['category_ids', 'colors', 'attributes', 'choice_attributes']))
                                                            {{ \Illuminate\Support\Str::limit(json_encode($oldVal), 100) }}
                                                        @else
                                                            {{ \Illuminate\Support\Str::limit($oldVal, 100) }}
                                                        @endif
                                                    </td>
                                                    <td class="text-success">
                                                        @if(in_array($key, ['category_ids', 'colors', 'attributes', 'choice_attributes']))
                                                            {{ \Illuminate\Support\Str::limit(json_encode($newVal), 100) }}
                                                        @else
                                                            {{ \Illuminate\Support\Str::limit($newVal, 100) }}
                                                        @endif
                                                    </td>
                                                </tr>
                                            @endif
            ...
            <div class="col-md-4">
                ...
                @if($changeRequest->status == 'pending')
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <form action="{{ route('admin.product.approve-change-request') }}" method="POST" class="mb-3">
                                @csrf
                                ...
                                <button type="submit" class="btn btn-success btn-block">
                                    <i class="tio-checkmark-circle"></i> {{ \App\CPU\translate('Approve & Apply Changes') }}
                                </button>
                            </form>
                ...
                @if($changeRequest->product)
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <p><strong>{{ \App\CPU\translate('Price') }}:</strong> {{ \App\CPU\BackEndHelper::currency_set_symbol(\App\CPU\BackEndHelper::usd_to_currency($changeRequest->product->unit_price)) }}</p>
                            <p><strong>{{ \App\CPU\translate('Status') }}:</strong>
                                @if($changeRequest->product->approval_status == 'approved')
                                    <label class="badge badge-soft-success">{{ \App\CPU\translate('Approved') }}</label>
                                @elseif($changeRequest->product->approval_status == 'pending_edit')
                                    <label class="badge badge-soft-warning">{{ \App\CPU\translate('Pending Edit') }}</label>
                                @else
                                    <label class="badge badge-soft-secondary">{{ $changeRequest->product->approval_status }}</label>
                                @endif
                            </p>
                        </div>
                    </div>
