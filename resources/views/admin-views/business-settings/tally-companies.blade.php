...
@section('title', \App\CPU\translate('Tally Companies'))
    ...
    <div class="content container-fluid">
        ...
        <div class="mb-3">
            <h2 class="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
                ...
                {{ \App\CPU\translate('Tally Companies') }}
            </h2>
        </div>
        <!-- End Page Title -->
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                        ...
                        <div class="d-flex align-items-center gap-2 mt-2 mt-sm-0">
                            ...
                            <form action="{{ route('admin.business-settings.tally-companies.toggle-sync') }}" method="POST" id="tally_sync_form">
                                @csrf
                                <label class="switcher mx-auto">
                                    <input type="checkbox" class="switcher_input" name="tally_sync" value="1" {{ auth('admin')->user()->tally_sync ? 'checked' : '' }} onchange="document.getElementById('tally_sync_form').submit()">
                                        ...
                                        @foreach($companies as $k => $company)
                                            <tr class="{{ $active_company == $company->company_name ? 'bg-soft-primary' : '' }}">
                                                ...
                                                <td>
                                                    ...
                                                    @else
                                                        <a href="{{ route('admin.business-settings.tally-companies.select-company', $company->company_name) }}"
                        ...
                        @else
                            <div class="text-center p-5">
                                ...
                                <p class="mb-0 text-muted">
                                    {{ \App\CPU\translate('Please make sure Tally is running and the URL is configured correctly in your .env file (TALLY_URL).') }}
                                </p>
                            </div>
                        @endif
