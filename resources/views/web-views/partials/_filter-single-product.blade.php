...
<div class="bh-pc-card">
    ...
    <div class="bh-pc-body">
        ...
        <div class="bh-pc-price-row">
            <span class="bh-pc-selling">{{ \App\CPU\Helpers::currency_converter($sellingPrice) }}</span>
            @if ($product->discount > 0)
                <span class="bh-pc-mrp">{{ \App\CPU\Helpers::currency_converter($product->unit_price) }}</span>
            @endif
        </div>
        ...
        <div class="bh-pc-size-row">
            ...
            @if (Request::is('product/*'))
                <a class="bh-pc-size-btn" href="{{ route('product', $product->slug) }}">
                    <span>{{ $variations[0]['type'] ?? 'View' }}</span>
