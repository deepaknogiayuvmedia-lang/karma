                ...
                <div class="table-responsive">
                    <table class="table __table mb-0">
                        ...
                        <tbody>
                            <tr>
                                <td>
                                    {{ isset($ticket['created_at']) ? \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $ticket['created_at'])->format('Y-m-d') : 'N/A' }}
                                </td>
                                <td>
                                    {{ isset($ticket['updated_at']) ? \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $ticket['updated_at'])->format('Y-m-d') : 'N/A' }}
                                </td>
                                <td>
                                    {{isset($ticket['type']) ?$ticket['type'] : 'N/A'}}
                                </td>
                                <td>
                                    {{isset($ticket['priority']) ?$ticket['priority'] : 'N/A'}}
                                </td>
                                <td>
                                    @if(isset($ticket['status']) && $ticket['status']=='open')
                                    <span class="badge btn btn-secondary">{{$ticket['status']}}</span>
                                    @else
                                    <span class="badge btn btn-secondary">{{isset($ticket['status']) ? $ticket['status'] : 'N/A'}}</span>
                                    @endif
                                </td>
                            </tr>
                        </tbody>
                ...
                <div class="__media-wrapper card-body">
                    <div class="media __incoming-msg">
                        <img class="rounded-circle __img-40" style="text-align: {{Session::get('direction') === "rtl" ? 'left' : 'right'}};"
                            ...
                            src="{{asset(config('app.public_storage_path').'/profile')}}/{{auth('customer')->user()->image}}"
